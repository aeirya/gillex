...
The current entries are selected words extracted from \citet{rastorgueva2012gilaki} which have been segmented, analysed and split between the tables by a native speaker following the database structure described in this article. 
...
Gillex is intended as a public language resource. Code and scripts are under the MIT License. The original lexical data created by the project is released under CC BY 4.0. Source-derived material is limited to lexical facts, short normalized forms, and citations compatible with the source materials.
